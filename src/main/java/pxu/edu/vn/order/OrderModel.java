...
public class OrderModel {
    ...
    public static void insertOrder(Order order) throws Exception {
        ...
        try {
            conn = DBConnection.getConnection();
            stmt = conn.createStatement();
            String sql = "INSERT INTO orders (user_id, order_date, total_amount) VALUES (" + order.getUser_id() + ", '"
                    + order.getOrder_date() + "', " + order.getTotal_amount() + ")";
    ...
    public static void updateOrder(Order order) throws Exception {
        ...
        try {
            conn = DBConnection.getConnection();
            stmt = conn.createStatement();
            String sql = "UPDATE orders SET user_id = " + order.getUser_id() + ", order_date = '" + order.getOrder_date()
                    + "', total_amount = " + order.getTotal_amount() + " WHERE order_id = " + order.getOrder_id();
            stmt.executeUpdate(sql);
        } catch (SQLException e) {
            // Ghi log hoặc ném ra ngoại lệ chứa thông báo lỗi
